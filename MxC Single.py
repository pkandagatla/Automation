...
# def create_df_from_sheet(sheet_name,excelname):
#     Template=load_workbook(excelname,data_only=True)
...
#Returns excel format cell range
...
path_folder = r"C:\Users\pkandagatla\Desktop\Automation Single Assessment"

template_path = ""
payer_grp_send = ""

#If the script is called this will run
folder_path = sys.argv[1]
folder_path=folder_path.replace("\\","/").replace("\"","")

#If the script run directly we get the inputs
...
# file_saving_location = f"C:/Users/{username}/Downloads/AR Assessment - {D_Facility_Name}.xlsx"
payer_grp_mail = f"C:/Users/{username}/Downloads/AR Assessment - {folder_name} - Payer Grouping.xlsx"


#Checking the files required i.e setup files required and storing
for filename in os.listdir(path_folder):
    file_path = os.path.join(path_folder, filename)
    if filename.endswith("Assessment - MxC Single facility.xlsx"):
        template_path = file_path
    elif filename.endswith("AR Assessment - Payer Grouping.xlsx"):
        payer_grp_send = file_path
...
print(f"Transaction Report: {xls_file}")
print(f"Deposit Report: {xls_file_deposit}")
print(f"DSO Report: {xls_file_dso}")
print(f"Aging report: {xls_file_aging}")
print(f"Payer group: {payer_group}")


print("Started Raw Data DF Creation")

#-------------------------Creating Dataframes with the raw data files----------------------------------
#-------------Transaction-----------------
...
transaction_report_df= read_rawdata(xls_file,TreportCnames)
...
Deposit_report_df=read_rawdata(xls_file_deposit,DreportCnames)
...
Aging_report_df=read_rawdata(xls_file_aging,AreportCnames)
...
print("Raw Data transformed to Dataframes")
...
for i in range(len(Aging_report_dfheaders)):
    if(Aging_report_dfheaders[i]==Aging_report_df.iloc[0,i]):
        check+=1
if(check!=11):
    print(Aging_report_df.iloc[0])
    aging_access=input(r"Can we proceed with this Age buckets as they are in unsual format, can we continue assessment(Yes/No)= ")
    if(aging_access!="Yes"):
        print("Please use the correct Aging report to continue the Assessment")
        

#Facility Name check
Facility_access="Yes"
if(D_Facility_Name==T_Facility_Name==DSO_report_df.iloc[1,1]==A_Facility_Name):
    print("Assessment for ",T_Facility_Name," is loading!")
else:
    print("Transaction Report= ",T_Facility_Name,"\nDeposit Report= ",DSO_report_df.iloc[1,1],"\nAging Report= ",A_Facility_Name, "\nDSO Report= ",DSO_Facility_Name)
    Facility_access=input(r"Facility Names are mismatching b/w reports, is it fine to proceed? (Yes/No)= ")
    if(Facility_access!="Yes"):
        print("Please use corresponding files for Assessment")
        
#Date Range Check
Date_range_access="Yes"
t_date_range=transaction_report_df2.iloc[2,1]
start_index = t_date_range.find(":") + 2
end_index = t_date_range.find("    ")

t_date_range = t_date_range[start_index:end_index]
d_date_range= Deposit_report_df_2.iloc[4,4]
if(t_date_range!=d_date_range):
    print("We have different Date Ranges in Transaction and Deposit Report \n Transaction Report= ",t_date_range,"\n Deposit Report= ",d_date_range)
...
aging_access2="Yes"
aging_check=iter_row_slicer(Aging_report_df2,"Payer Type Summary",1)
if(len(aging_check)==16):
    print("We don't have Payer Type Summary for the Assessment")
    aging_access2=input(r"Do you wish to only run the assessment for NCR & without Aging? (Yes/No)")
    if(aging_access2!="Yes"):
        print("Get the corrected Aging report to run the assessment")
        
#Age Through and Cash Through Date Check
aging_date_check="Yes"
age_through_date=Aging_report_df2.iloc[2,1]
cash_through_date=Aging_report_df2.iloc[3,1]
...
if(age_through_date!=cash_through_date):
    print("Age Through Date and Cash Through Date are not matching")
    print("Age Thru Date= ",age_through_date,"\n Cash Through Date= ",cash_through_date)
    aging_date_check=input(r"Do you want to continue with this age through and cash through date?(Yes/No)")
    if(aging_date_check!="Yes"):
        print("Get the corrected Aging report to run the assessment")    
        
        
if(aging_access==Facility_access==Date_range_access==aging_access2==aging_date_check):
    print("Assessment Satisfied all the raw data checks!")

elif("No" in [aging_access,aging_access2,aging_date_check]):
    Aging_omission=False
    ncr_only=input(r"Can we continue assessment with omitting Aging Cal? (Yes/No)")
    if(ncr_only=="Yes"):
        Aging_omission=True
    else:
        print("Get the appropriate files to run the assessment")
        sys.exit()
else:
    sys.exit()
...
for i in range(0,len_t_report,500000):
    ...
    print("Converting Template to Dataframe")
    ...
    print("Dataframe created, loading workbook")
    
    #Loading workbook
    Template=load_workbook(filename=template_path)
    
    print("Converted and loaded the excel, execution starts....")
    ...
    Template=copy_paste_raw(Template,"Transaction",transaction_report_df,row_no+1,col_no,to_col)
    ...
    Template=border_apply_bottom_u_type(Template,"Transaction",row_no,len(transaction_report_df)+row_no,col_no,col_no)
    Template=border_apply_bottom_u_type(Template,"Transaction",row_no,len(transaction_report_df)+row_no,col_no+1,to_col-1)
    Template=border_apply_bottom_u_type(Template,"Transaction",row_no,len(transaction_report_df)+row_no,to_col,for_to_col)
    
    #Extend Formulas in Sheets
    Template=extend_formulas(Template,"Transaction",transaction_report_df,for_row_no+2,for_col_no,for_to_col)
    print("Transaction report executed")
    ...
    #Paste values in excel
    Template=copy_paste_raw(Template,"Deposit",Deposit_report_df,row_no+1,col_no,to_col)
    ...
    #Extend Formulas in Sheets
    Template=extend_formulas(Template,"Deposit",Deposit_report_df,for_row_no+2,for_col_no,for_to_col)
    Template=border_apply_bottom_u_type(Template,"Deposit",row_no,len(Deposit_report_df)+row_no,col_no,col_no)
    Template=border_apply_bottom_u_type(Template,"Deposit",row_no,len(Deposit_report_df)+row_no,col_no,to_col)
    
    test_row,test_col=coordinate_finder_insheet(template_deposit,"Test",1)
    Template=border_apply_bottom_u_type(Template,"Deposit",row_no,len(Deposit_report_df)+row_no,for_col_no,test_col-1)
    Template=border_apply_bottom_u_type(Template,"Deposit",row_no,len(Deposit_report_df)+row_no,test_col,for_to_col)
    
    print("Deposit report executed")
    ...
    #Paste values in excel
    Template=copy_paste_raw(Template,"Agings",Aging_report_df,row_no,col_no,to_col)
    ...
    #Paste values in excel
    Template=copy_paste_raw(Template,"DSO",DSO_report_df,row_no+2,col_no-1,col_no+13)
    
    print("Raw Data pasting and formula extensions are completed! ")
    
    
    #Calculating time to paste raw data
    raw_data_end_time=time.time()
    
    elapsed_time_seconds = raw_data_end_time - raw_data_start_time
    ...
    Template=paste_value_in_cell(Template,"DSO & NCR",row_no,col_no+19,end_date)
    
    #----------AR Clean Up Estimates----------------------------
    Aging_report_df2=Aging_report_df.iloc[1:len(Aging_report_df)-1,:]
    Aging_report_df2
    
    #Unique list of payers need to be added in the AR Clean up estimates
    wanted_list=unique_values_except(Aging_report_df2, ["Private", "Other", "Miscellaneous"], 1)
    wanted_list=pd.DataFrame(wanted_list)
    
    #Getting position
    row_num, col_num=coordinate_finder_insheet(template_clean_up,"Payer Type",1)
    ...
    #Getting position
    row_num, col_num=coordinate_finder_insheet(template_payments,"Payer Group",1)
    row_end, col_end=coordinate_finder_insheet(template_payments,"Overall",1)
    
    #Paste values in excel
    Template=copy_paste_raw(Template,"Payments",uniques_payer_groups,row_num+1,col_num,col_num)
    ...
    #---------------------Aging Cal-------------------------
    no_aging_payers=len(Aging_report_df)-2
    
    #Getting position
    row_no,col_no=coordinate_finder_insheet(template_aging_cal,"Payer Type Summary",1)
    row_end,col_end=coordinate_finder_insheet(template_aging_cal,"Payer Type Total",1)
    
    #Calculation to find the start row nd number of rows to delete
    start_row=row_no+no_aging_payers+1
    no_of_row=row_end-start_row
    
    #Delete unwated rows
    worksheet_aging_cal=deleting_below_rows(worksheet_aging_cal,start_row,no_of_row)
    ...
    if(no_months<18):
        worksheet_dsoncr=deleting_right_columns(worksheet_dsoncr,col_no+2,18-no_months)
    
    #Here finding the empty cell and then we can delete from there
    row_no,col_no=coordinate_finder_insheet(template_dsoncr,"Metrics",no_of_payer_grps+2)
    
    worksheet_dsoncr=deleting_below_rows(worksheet_dsoncr,row_no,10000)
    ...
    #Delete unwated rows
    worksheet_clean_up=deleting_below_rows(worksheet_clean_up,start_row,no_of_row)
    ...
    #Calculation to find the start row nd number of rows to delete
    start_row=row_num+len(uniques_payer_groups)+1
    no_of_row=row_end-start_row
    
    #Delete unwated rows
    worksheet_payments=deleting_below_rows(worksheet_payments,start_row,no_of_row)
    ...
    # row_no_metrics,col_no_metrics=coordinate_finder_insheet(template_dsoncr,"Metrics",1)
    
    # #geting range to move the cols and rows
    # if(no_months<18):    
    #     start_row=row_no-2
    #     start_col=col_no-(18-no_months)
    #     end_col=col_no+14-(18-no_months)
    #     end_row=row_no+1
    #     columns_to_move=col_no_metrics-col_no-(18-no_months)
    # else:
    #     start_row=row_no-2
    #     start_col=col_no
    #     end_col=col_no+14
    #     end_row=row_no+1
    #     columns_to_move=col_no_metrics-col_no
